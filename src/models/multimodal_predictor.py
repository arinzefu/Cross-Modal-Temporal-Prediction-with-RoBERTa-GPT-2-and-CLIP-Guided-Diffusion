...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x):
        """x: [B, T, D] -> pooled [B, D], weights [B, T, 1]"""
...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, vision_seq, text_seq):
        """vision_seq: [B, Tv, vision_dim], text_seq: [B, Tt, text_dim]"""
...
class MultiScaleTemporalTransformer(nn.Module):
    def __init__(self, dim, num_heads=8, num_layers=4, dropout=0.1):
        super().__init__()

        # FIX: build a *fresh* encoder layer for each scale. The previous version
        # reused one layer instance across all scales, so every scale shared the
        # same weights (defeating the multi-scale idea).
...
# (kept for compatibility; not used by the diffusion-based predictor,
#  which uses a global CLIP latent rather than a 14x14 spatial map)
# =========================================================
class SpatialFeatureProjector(nn.Module):
    def __init__(self, in_channels=256, hidden_channels=256, output_dim=512):
        super().__init__()
